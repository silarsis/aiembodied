...
- 2025-10-23 — Secret submission handlers in the renderer now flush success/error status updates synchronously so Vitest can observe the confirmation message. Renderer tests use `findByText` when asserting the success notice to avoid timing flake in CI.
