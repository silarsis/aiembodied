...
- 2025-10-21 — Avatar face uploads now run a Responses API descriptor analysis before image generation. Schema fields (hair/eye color, accessories, notable features) must flow into every `images.create` prompt. Tests mock `responses.create` via `output_text` JSON payloads; update them alongside service changes.
