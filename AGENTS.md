...
- 2025-01-14 — Main test suite split into separate sequential runs to prevent heap exhaustion. Tests run in groups (config, avatar, VRMA, etc.) within separate Node processes. Each group is isolated with `isolate: true` in vitest config. `pnpm test` and `pnpm test:coverage` now use split scripts (`scripts/test-split.mjs` and updated `scripts/test-coverage.mjs`).
